...
with model:
    ...
    pc_x = nengo.Probe(posecells_x)
    pc_node = nengo.Probe(posecell_node)
    x_spikes = nengo.Probe(posecells_x.neurons, 'spikes')
    y_spikes = nengo.Probe(posecells_y.neurons, 'spikes')
    th_spikes = nengo.Probe(posecells_th.neurons, 'spikes')


    print("I am here")
    #print("spin_pc_x", pc_x)
    #np.save("spin_pc_x", sim.data[pc_x])
    #np.save("spin_pc_node", sim.data[pc_node])
    #np.save("spin_x_spikes", sim.data[x_spikes])
    #np.save("spin_y_spikes", sim.data[y_spikes])
    #np.save("spin_th_spikes", sim.data[th_spikes])
...
if __name__ == '__main__':
    ...
    #plt.subplot(2, 1, 1)
    plt.plot(sim.trange(), sim.data[x_spikes][:,1:3])

    #plt.subplot(2, 1, 2)
    #plt.plot(sim.trange(), sim.data[y_spikes][1:50])
    plt.xlabel("Time [s]")
