...
from vitaslam.msg import CombinedTemplateMessage, TopologicalAction
import cv2
...
PC_C_SIZE_ABG            = (2.*np.pi)/PC_DIM_ABG
PC_E_XY_WRAP            = range(PC_DIM_XYZ-PC_W_E_DIM_HALF, PC_DIM_XYZ) + range(PC_DIM_XYZ) + range(PC_W_E_DIM_HALF)
PC_E_TH_WRAP            = range(PC_DIM_ABG-PC_W_E_DIM_HALF, PC_DIM_ABG) + range(PC_DIM_ABG) + range(PC_W_E_DIM_HALF)
PC_I_XY_WRAP            = range(PC_DIM_XYZ-PC_W_I_DIM_HALF, PC_DIM_XYZ) + range(PC_DIM_XYZ) + range(PC_W_I_DIM_HALF)
PC_I_TH_WRAP            = range(PC_DIM_ABG-PC_W_I_DIM_HALF, PC_DIM_ABG) + range(PC_DIM_ABG) + range(PC_W_I_DIM_HALF)            
PC_XY_SUM_SIN_LOOKUP    = np.sin(np.multiply(range(1, PC_DIM_XYZ+1), (2*np.pi)/PC_DIM_XYZ))
PC_XY_SUM_COS_LOOKUP    = np.cos(np.multiply(range(1, PC_DIM_XYZ+1), (2*np.pi)/PC_DIM_XYZ))
PC_TH_SUM_SIN_LOOKUP    = np.sin(np.multiply(range(1, PC_DIM_ABG+1), (2*np.pi)/PC_DIM_ABG))
PC_TH_SUM_COS_LOOKUP    = np.cos(np.multiply(range(1, PC_DIM_ABG+1), (2*np.pi)/PC_DIM_ABG))
PC_CELLS_TO_AVG         = 3
PC_AVG_XY_WRAP          = range(PC_DIM_XYZ-PC_CELLS_TO_AVG, PC_DIM_XYZ) + range(PC_DIM_XYZ) + range(PC_CELLS_TO_AVG)
PC_AVG_TH_WRAP          = range(PC_DIM_ABG-PC_CELLS_TO_AVG, PC_DIM_ABG) + range(PC_DIM_ABG) + range(PC_CELLS_TO_AVG)


class PosecellNetwork(object):

    def __init__(self, root='/whiskeye',
                 PC_DIM_XYZ=21, PC_DIM_ABG=36, pc_w_e_dim=7,
        ...
        print(self.root + '/PoseCell/TopologicalAction')
        ...
        self.best_x = np.floor(PC_DIM_XYZ / 2.0)
        self.best_y = np.floor(PC_DIM_XYZ / 2.0)
        self.best_z = np.floor(PC_DIM_XYZ / 2.0)
        self.best_a = np.floor(PC_DIM_ABG / 2.0)
        self.best_b = np.floor(PC_DIM_ABG / 2.0)
        self.best_g = np.floor(PC_DIM_ABG / 2.0)
    ...
    def on_view_template(self, vt, vt_rad):
        ...
        if vt >= len(self.visual_templates):
            ...
            if vt < len(self.visual_templates) - 10:
                ...
                if energy > 0:
                    self.vt_delta_pc_th = vt_rad / (2.0*np.pi) * PC_DIM_ABG
                    pc_th_corrected = pcvt['g_pc'] + vt_rad / (2.0*np.pi) * PC_DIM_ABG
                    if pc_th_corrected < 0:
                        pc_th_corrected += PC_DIM_ABG
                    if pc_th_corrected >= PC_DIM_ABG:
                        pc_th_corrected -= PC_DIM_ABG
    ...
    def create_experience(self):
        #print("I am inside create experience")
    ...
    def pose_cell_builder(self):
        ...
        self.pc_c_size_th = 2 * np.pi / PC_DIM_ABG
        self.posecells = np.zeros((PC_DIM_XYZ, PC_DIM_XYZ, PC_DIM_ABG))

        self.posecells[np.floor(PC_DIM_XYZ/2), 
                       np.floor(PC_DIM_XYZ/2),
                       np.floor(PC_DIM_ABG/2)] = 1
    ...
    def get_relative_rad(self):
        ...
        return self.vt_delta_pc_th * 2.0 * np.pi / PC_DIM_ABG

    def inject(self, act_x, act_y, act_z, energy):

        if ((act_x < PC_DIM_XYZ) & (act_x >= 0) & (act_y < PC_DIM_XYZ) & (act_y >= 0) & (act_z < PC_DIM_ABG) & (act_z >= 0)):
    ...
    def compute_activity_matrix(self, xywrap, thwrap, wdim, pcw): 
        ...
        pca_new = np.zeros([PC_DIM_XYZ, PC_DIM_XYZ, PC_DIM_ABG])
    ...
    def path_integration(self, vtrans, vrot):
        ...
        for dir_pc in xrange(PC_DIM_ABG): 
            direction = np.float64(dir_pc-1) * PC_C_SIZE_ABG
            ...
            if (direction == 0):
                ...
                pca_new = np.zeros([PC_DIM_XYZ+2, PC_DIM_XYZ+2])   
        ...
        if vrot != 0: 
            weight = (np.abs(vrot)/PC_C_SIZE_ABG)%1
            if weight == 0:
                weight = 1.0

            shift1 = int(np.sign(vrot) * int(np.floor(abs(vrot)/PC_C_SIZE_ABG)))
            shift2 = int(np.sign(vrot) * int(np.ceil(abs(vrot)/PC_C_SIZE_ABG)))
    ...
    def find_best(self):
        ...
        z_posecells = np.zeros([PC_DIM_XYZ, PC_DIM_XYZ, PC_DIM_ABG]) 
    ...
    def find_best_new(self):
        ...
        # get the sums for each axis
        x_sums = np.zeros(PC_DIM_XYZ)
        y_sums = np.zeros(PC_DIM_XYZ)
        z_sums = np.zeros(PC_DIM_ABG)
        ...
        x = np.arctan2(sum_x1, sum_x2) * PC_DIM_XYZ / (2.0 * np.pi) - 1.0
        while x < 0:
            x += PC_DIM_XYZ
        while x > PC_DIM_XYZ:
            x -= PC_DIM_XYZ

        y = np.arctan2(sum_y1, sum_y2) * PC_DIM_XYZ / (2.0 * np.pi) - 1.0
        while y < 0:
            y += PC_DIM_XYZ
        while x > PC_DIM_XYZ:
            y -= PC_DIM_XYZ
        ...
        sum_x1 = 0
        sum_x2 = 0
        for i in range(PC_DIM_ABG):
            sum_x1 += PC_TH_SUM_SIN_LOOKUP[i] * z_sums[i]
            sum_x2 += PC_TH_SUM_COS_LOOKUP[i] * z_sums[i]

        th = np.arctan2(sum_x1, sum_x2) * PC_DIM_ABG / (2.0 * np.pi) - 1.0
        while th < 0:
            th += PC_DIM_ABG
        while x > PC_DIM_ABG:
            th -= PC_DIM_ABG
    ...
    def get_action(self):
        ...
        if len(self.experiences) == 0:
            #print("len of experiences is 0")
            self.create_experience()
            action = CREATE_NODE
        else:
            experience = self.experiences[self.current_exp]

            delta_pc = self.get_delta_pc(experience['x_pc'], experience['y_pc'], experience['g_pc'])

            pcvt = self.visual_templates[self.current_vt]

            if len(pcvt['exps']) == 0:
                #print("len of pcvt[exps] is 0")
                self.create_experience()
                action = CREATE_NODE
                ...
                if self.current_exp != matched_exp_id:
                    ...
                    #print("self.current_vt == self.prev_vt")
                    self.create_experience()
                    action = CREATE_NODE
    ...
    def get_delta_pc(self, x, y, th):
        ...
        pc_th_corrected = self.best_g - self.vt_delta_pc_th
        ...
        return np.sqrt(self.get_min_delta(self.best_x, x, PC_DIM_XYZ)**2 +\
                       self.get_min_delta(self.best_y, y, PC_DIM_XYZ)**2 +\
                       self.get_min_delta(self.best_g, th, PC_DIM_ABG)**2
                      )
...
if __name__ == '__main__':
    ...
    pc_im = pc_ax.imshow(np.zeros((PC_DIM_XYZ, PC_DIM_XYZ)),
                                   #vmax=255, vmin=-255,
                                   vmax=1, vmin=0,
    ...
    """
    mx_p = []
    x = np.floor(PC_DIM_XYZ/2)
    y = np.floor(PC_DIM_XYZ/2)
    th = np.floor(PC_DIM_ABG/2)
    mx_p.append(mx_ax.plot3D([0, PC_DIM_XYZ], [y, y], [th, th], 'K'))
    mx_p.append(mx_ax.plot3D([x, x], [0, PC_DIM_XYZ], [th, th], 'K'))
    mx_p.append(mx_ax.plot3D([x, x], [y, y], [0, PC_DIM_ABG], 'K'))
    mx_p.append(mx_ax.plot3D([x], [y], [th], 'mo'))
    
    mx_ax.set_xlim3d([0, PC_DIM_XYZ])
    mx_ax.set_ylim3d([0, PC_DIM_XYZ])
    mx_ax.set_zlim3d([0, PC_DIM_ABG])
    ...
    while not rospy.is_shutdown():
        ...
        mx_p.append(mx_ax.plot3D([x], [y], [th], 'mo'))
        """
        mx_ax.plot3D([0, PC_DIM_XYZ], [y, y], [th, th], 'K')
        mx_ax.plot3D([x, x], [0, PC_DIM_XYZ], [th, th], 'K')
        mx_ax.plot3D([x, x], [y, y], [0, PC_DIM_ABG], 'K')
